...
void usage(int argc, char* argv[]) {
	...
	cout << "繁體中文支援 by Naruse (Email:admin@chackbox.com) " << endl;
    cout << "請前往檢視更新 (英語) http://www.elifulkerson.com/projects/ " << endl;
    cout << "--------------------------------------------------------------" << endl;
    cout << endl;
    cout << "基礎語法: " << argv[0] << " [-flags] 域名或者IP地址(google.cn) [伺服器連接埠(80或者443)]" << endl << endl;
    cout << "完整語法: " << argv[0] << " [-t] [-d] [-i interval] [-n times] [-w ms] [-b n] [-r times] [-s] [-v] [-j] [-js size] [-4] [-6] [-c] [-g count] [-S source_address] [--file] [--tee filename] [-h] [-u] [--post] [--head] [--proxy-port port] [--proxy-server server] [--proxy-credentials username:password] [-f] server-address " << "[server-port]" << endl << endl;
    cout << " -t     : 連續ping直到通過control-c(用戶發出終止命令)停止" << endl;
    cout << " -n 5   : 發送5次tcping" << endl;
    cout << " -i 5   : 間隔5秒執行tcping" << endl;
    cout << " -w 0.5 : 等待0.5秒響應發出的請求" << endl;
    cout << " -d     : 每行發出的命令帶有日期和時間" << endl;
    cout << " -b 1   : enable beeps (1 for on-down, 2 for on-up," << endl;
    cout << "                        3 for on-change, 4 for always)" << endl;
    cout << " -r 5   : 間隔5次tcping之後重新查找一次主機名" << endl;
    cout << " -s     : 成功回應發出的tcping請求後自動退出"<<endl;                  //[Modification 14 Apr 2011 by Michael Bray, mbray@presidio.com]
    cout << " -v     : 打印當前的程式版本後退出" << endl;
    cout << " -j     : 使用預設的平均值包括抖動"<< endl;
	cout << " -js 5  : 使用預設的平均值,平均值的偏差在5左右,包括抖動" << endl;
	cout << " --tee  : 將輸出鏡像到'--tee'之後指定的文件名" << endl;
	cout << " --append : 附加到--tee文件名,但不覆蓋文件" << endl;
	cout << " -4     : 優先使用 ipv4" << endl;
	cout << " -6     : 優先使用 ipv6" << endl;
	cout << " -c     : 僅在更改狀態下顯示輸出線" << endl;
	...
	cout << " --fqdn : 在每一行上打印域名(如果有)" << endl;
	cout << " --ansi : 使用ANSI顏色順序 (cygwin)" << endl;
	cout << " --color: 使用Windows顏色序列" << endl;
	
    cout << endl << "HTTP 選項:" << endl;
    cout << " -h     : HTTP模式(使用不帶http://的url作為伺服器地址)" << endl;
    cout << " -u     : 在每行上包含目標URL" << endl;
    cout << " --post : 使用POST而不是GET (可以避免緩存)" << endl;
    cout << " --head : 使用HEAD而不是GET" << endl;
	cout << " --proxy-server : 指定代理伺服器 " << endl;
	cout << " --proxy-port   : 指定代理伺服器通訊埠 " << endl;
	cout << " --proxy-credentials : 以用戶名：密碼格式指定“代理授權：基本”標頭" << endl;
    cout << endl << "Debug (開發人員) 選項:" << endl;
    cout << " -f     : 強制tcping發送至少一個字節" << endl;
	cout << " --header : 包括帶有原始參數和日期的標頭。 如果使用--tee則隱含" << endl;
